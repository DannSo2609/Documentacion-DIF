# Documentación del Plugin DC_Notificacion_Embarque
...
El plugin `DC_Notificacion_Embarque` automatiza el envío de notificaciones a usuarios o equipos responsables cuando se les asigna un registro de Embarque en Dynacorp (Dynamics 365). Su objetivo es garantizar que los responsables reciban una alerta inmediata y personalizada, facilitando el seguimiento y la gestión eficiente de embarques dentro del sistema.

---

## Propósito

- Notificar automáticamente a los responsables (usuarios o equipos) cuando se les asigna un registro de Embarque.
...
- Extrae el campo `ownerid` del registro de Embarque para identificar si el responsable es un usuario individual o un equipo.
...
- Incluye un enlace directo al registro de Embarque asignado y un ícono personalizado para la notificación.
...
- **Automatización:** Elimina la necesidad de notificaciones manuales al asignar registros de Embarque.
...
- **Asignación de registros de Embarque:** Cuando se asigna un registro a un usuario o equipo, estos reciben una notificación automatizada para iniciar el seguimiento.
...
El plugin `DC_Notificacion_Embarque` es esencial para la gestión eficiente de asignaciones y la comunicación interna en Dynacorp, asegurando que usuarios y equipos sean notificados de manera oportuna y personalizada ante nuevas responsabilidades en registros de embarque.